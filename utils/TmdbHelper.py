...
def process_info(info):
    target_element = {}
    target_element_list = ['budget', 'revenue', 'production_companies', 'production_countries', 'runtime']
...
def get_movies(start_page: int = 1, end_page: int = 30):
    session = requests_cache.CachedSession('movie_cache')
    page_movies = []
    for i in range(start_page, end_page):
        try:
            url = (f"https://api.themoviedb.org/3/discover/movie?include_video=false&language=en-US&page={i}&sort_by"
                   f"=popularity.desc")
...
def lang_transform(code, codes):
    if code in codes['639-1'].tolist():
        return codes.loc[codes['639-1'].isin([code])]['Language name(s) from ISO 639-2[1]']\
